...
.hero .btn-outline:hover {
  ...
  color: var(--color-accent);
      ...
      @media (max-width: 640px) {
        ...
        /* Full-width, stacked CTAs with a real touch height — a mobile
           composition, not two desktop buttons squeezed onto one row. */
        .hero-actions {
          flex-direction: column;
          align-items: stretch;
        }
      ...
      /* Product grid — a real CSS grid on tablet/desktop; below 768px it
         becomes a horizontal swipe carousel (one main product + a hint of
         the next) instead of a cramped multi-column grid. */
      .product-grid {
        display: grid;
        grid-template-columns: repeat(auto-fill, minmax(220px, 1fr));
        gap: var(--space-5);
